...
window.addEventListener('resize', logScrollbarWidth);
document.addEventListener('DOMContentLoaded', logScrollbarWidth);
...
fetch('navbar.html')
    ...
    .then(data => {
        // Inject navbar into both header elements
        document.querySelector('#main-header').innerHTML = data;


        // Ensure GSAP runs after elements are injected
        requestAnimationFrame(() => {
